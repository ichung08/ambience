...
const AudioPlayer = ({ tracks, mood }) => {
    ...
    // Destructure ofr conciseness
    const { title, artist, color, image, audioSrc } = tracks[mood][trackIndex]
    ...
    const toPrevTrack = () => {
        if (trackIndex - 1 < 0) {
            setTrackIndex(tracks[mood].length - 1)
        } else {
    ...
    const toNextTrack = () => {
        if (trackIndex < tracks[mood].length - 1) {
            setTrackIndex(trackIndex + 1)
        } else {
            setTrackIndex(0)
        }
